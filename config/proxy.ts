...
export const proxyConfig = {
  ...
  '/api/conversion': {
    ...
    configure: (proxy: any) => {
      proxy.on('error', (err: any, req: any, res: any) => {
        console.error('转化事件API代理错误:', err);
        if (!res.headersSent) {
          res.writeHead(500, { 'Content-Type': 'application/json' });
        }
        res.end(JSON.stringify({ code: 500, message: '转化事件服务连接失败' }));
      });

      proxy.on('proxyReq', (proxyReq: any) => {
        console.log('🔄 代理转化事件API请求:', proxyReq.method, proxyReq.path);
      });

      proxy.on('proxyRes', (proxyRes: any, req: any) => {
        console.log('✅ 代理转化事件API响应:', proxyRes.statusCode, req.url);
      });
    },
  },
  ...
  '/api/entity': {
    ...
    configure: (proxy: any) => {
      proxy.on('error', (err: any, req: any, res: any) => {
        console.error('主体管理API代理错误:', err);
        if (!res.headersSent) {
          res.writeHead(500, { 'Content-Type': 'application/json' });
        }
        res.end(JSON.stringify({ code: 500, message: '主体管理服务连接失败' }));
      });

      proxy.on('proxyReq', (proxyReq: any) => {
        console.log('🔄 代理主体管理API请求:', proxyReq.method, proxyReq.path);
      });

      proxy.on('proxyRes', (proxyRes: any, req: any) => {
        console.log('✅ 代理主体管理API响应:', proxyRes.statusCode, req.url);
      });
    },
  },
};